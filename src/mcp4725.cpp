...
bool MCP4725::begin()
{
  ...
  if (! isConnected())
  {
      ESP_LOGE("MCP4725", "Device not connected at address 0x%02x", _deviceAddress);
      return false;
  } 
...
{
  return i2c.checkAddr(_deviceAddress);
...
int MCP4725::_writeFastMode(const uint16_t value)
{
  ...
  return i2c.writeWord(_deviceAddress, dat);
}
...
int MCP4725::_writeRegisterMode(const uint16_t value, uint8_t reg)
{
  ...
  return i2c.writeWordReg(_deviceAddress, reg, dat);
...
uint8_t MCP4725::_readRegister(uint8_t* buffer, const uint8_t length)
{
  ...
  //}
  //return readBytes;
  cnt = i2c.readNBytes(_deviceAddress, buffer, length);
  if(cnt < 0) return 0;
  else return cnt;
}
